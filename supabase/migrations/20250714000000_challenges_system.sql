...
DROP POLICY IF EXISTS "Users can create their own attempts" ON public.user_challenge_attempts;
CREATE POLICY "Users can create their own attempts" 
ON public.user_challenge_attempts 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own attempts" ON public.user_challenge_attempts;
...
CREATE INDEX IF NOT EXISTS idx_user_challenge_attempts_user_id ON public.user_challenge_attempts(user_id);
CREATE INDEX IF NOT EXISTS idx_user_challenge_attempts_challenge_id ON public.user_challenge_attempts(challenge_id);
...
LIMIT 2
ON CONFLICT (challenge_date, challenge_id) DO NOTHING; 